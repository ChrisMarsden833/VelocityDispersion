...
extern "C"
{
                           ...
                           int size)
    {
        ...
        int progress = 0;
        #pragma omp parallel for default(none) shared(progress, size, res, Aperture, redshift, bulge_mass, bulge_radius, bulge_beta, bulge_sersicIndex, componentFlag, disk_mass, disk_inclination, Halo_mass, profile_name, halo_concentration, BlackHole_mass, size, stdout) schedule(dynamic, 1)
        for (int i = size-1; i >= 0; i--)
        {
            res[i] = GetVelocityDispersion(Aperture[i], redshift[i], bulge_mass[i], bulge_radius[i], bulge_beta[i], bulge_sersicIndex[i], componentFlag,
                    disk_mass[i], disk_inclination[i], Halo_mass[i], profile_name, halo_concentration[i] , BlackHole_mass[i]);
